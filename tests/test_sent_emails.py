...
def test_only_the_requested_address_and_topic_are_considered(database_file: Path):
    ...
    {'address': 'someone_else@somewhere.com', 'topic': 'Something', 'sent_at': datetime(2020, 9, 15, 18, 0, 0)},
    {'address': 'someone@somewhere.com', 'topic': 'Something else', 'sent_at': datetime(2020, 9, 15, 19, 0, 0)},
    {'address': 'someone@somewhere.com', 'topic': 'Something', 'sent_at': datetime(2020, 10, 15, 17, 0, 0)},
    {'address': 'someone-else@somewhere.com', 'topic': 'Something', 'sent_at': datetime(2020, 10, 15, 18, 0, 0)},
    {'address': 'someone@somewhere.com', 'topic': 'Something else', 'sent_at': datetime(2020, 10, 15, 19, 0, 0)}
    ]
...
def test_sent_emails_are_returned_ordered_by_datetime(database_file: Path):
    ...
    unordered_times = [ordered_times[3], ordered_times[2], ordered_times[7], ordered_times[5], ordered_times[6], ordered_times[0], ordered_times[1], ordered_times[4]]
...
def test_last_sent_at_returns_datetime_when_last_sent(database_file: Path):
    ...
    data = [
        ...
        {'address': "someone@somewhere.com", 'topic': "Something", 'sent_at': datetime(2020, 4, 8, 0, 0, 0)},
        {'address': "someone@somewhere.com", 'topic': "Something", 'sent_at': datetime(2020, 4, 7, 0, 0, 0)},
        {'address': "someone-else@somewhere.com", 'topic': "Something", 'sent_at': datetime(2020, 4, 9, 0, 0, 0)}
    ]
    for d in data:
